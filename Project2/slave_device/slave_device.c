...
int slave_open(struct inode *inode, struct file *filp)
{
    ...
    if( (filp->private_data = kmalloc(MMAP_SIZE, GFP_KERNEL)) == NULL )
        printk("kmalloc failed");
    // if (kmem_ori = kmalloc(2 * MMAP_SIZE, GFP_KERNEL)){
    //     filp->private_data = PAGE_ALIGN((unsigned long)kmem_ori);
    // }
    return 0;
}
...
static long slave_ioctl(struct file *file, unsigned int ioctl_num, unsigned long ioctl_param)
{
    ...
    switch(ioctl_num){
        ...
        case slave_IOCTL_MMAP:
            /* receive new data from master device */
...
static int slave_mmap(struct file *filp, struct vm_area_struct *vma)
{
    /* For the explanation of the parameters, please refer to https://linux-kernel-labs.github.io/refs/heads/master/labs/memory_mapping.html
       filp: a pointer to a struct file created when the device is opened from user space
       vma: the virtual address space where the memory should be mapped by the device
       example: https://github.com/paraka/mmap-kernel-transfer-data/blob/master/mmap-example.c
    */
    ...
    /* remap_pfn_range() maps the physical memory starting at page fram number pfn
       to the vma->vm_start virtual address
    */
    int ret = remap_pfn_range(vma, vma->vm_start, pfn, len, vma->vm_page_prot);
    if (ret != 0) {
        printk(KERN_ERR "Error in calling remap_pfn_range");
        return -EAGAIN;
    }
